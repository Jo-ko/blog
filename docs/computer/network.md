...
2. 数据链路层: 处理比特流, 将比特流与帧进行转换(确定0与1的分组方式), 通过 __差错控制__ 提供可靠的数据链路传输, 包括**MAC物理寻址**,链路管理,数据的分装成帧,流量控制,差错控制,数据的校验重发,设备包括二层交换机,网桥.
3. 网络层: 处帧和数据包(包括源站点和目标站点的网路地址)之间的转换,选择合适的网间路由和交换节点,确保数据及时传输,建立主机-主机之间的连接,同时引入**ip逻辑选址**,用于区分哪些主机属于同一子网中,设备包括路由器.
...
- 序列号(seq): 本报文段发送数据的第一个字节的编号,是一个32位无符号数,SYN不为1时,是当前数据分段的第一个字母的序列号,SYN为1时,就是ISN
  - TCP报文可以一次发多个包,而网络延迟等一些原因会导致包抵达接收方的时候顺序混乱的情况,因此需要序列号进行重排序
...
2. 服务器接收到FIN请求后,会发送ACK确认报文,携带确认号ack=u+1,表示已经收到断开连接请求,并进入CLOSE_WAIT状态
...
- 当确认包发送三个相同ack标识的报文的时候,发送端就会在RTO的时间内(大部分是立即)重传丢失的包
...
### http2.0
1. 增加多路复用机制来解决http1.1的问题(对头阻塞问题并未完全解决)
2. 增加请求优先级设置
3. 服务器推送
4. 请求头和响应头压缩
...
1. 浏览器发送加密套件列表给服务端
2. 服务端选择一个加密套件并将生成的公匙返给客户端
3. 双方确认信息,客户端发送使用公匙加密的数据给服务端,服务端发送使用密匙加密的数据给客户端
...
2. 服务端收到后选择加密套件,同时生成随机数service-random和公匙,将这些发送给客户端
3. 客户端收到后生成pre-master,并利用公匙加密,发送给服务端
4. 服务端利用密匙解密获取到pre-master, 至此,客户端和服务端都拥有相同的client-random、service-random 和 pre-master, 并利用这个三个随机数生成对称密匙
...
[comment]: <> (参考)
[comment]: <> (https://cloud.tencent.com/developer/article/1629739)
